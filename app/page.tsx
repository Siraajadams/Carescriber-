...
import {useEffect,useState,type CSSProperties,} from "react";import Link from "next/link";import { supabase } from "../../lib/supabase";

type DoctorProfile = {id?: string;user_id?: string | null;

first_name?: string | null;surname?: string | null;last_name?: string | null;

email?: string | null;mobile?: string | null;phone?: string | null;

hpcsa?: string | null;registration_number?: string | null;

practice_number?: string | null;practice_address?: string | null;

qualifications?: string | null;qualification?: string | null;};

export default function DoctorProfilePage() {const [profileId, setProfileId] = useState("");const [authUserId, setAuthUserId] = useState("");

const [firstName, setFirstName] = useState("");const [surname, setSurname] = useState("");

const [email, setEmail] = useState("");const [originalEmail, setOriginalEmail] =useState("");

const [mobile, setMobile] = useState("");const [registrationNumber, setRegistrationNumber] =useState("");const [practiceNumber, setPracticeNumber] =useState("");const [practiceAddress, setPracticeAddress] =useState("");const [qualifications, setQualifications] =useState("");

const [loading, setLoading] = useState(true);const [saving, setSaving] = useState(false);const [message, setMessage] = useState("");

useEffect(() => {loadProfile();}, []);

async function loadProfile() {setLoading(true);setMessage("");
...
function validate() {const missing: string[] = [];
...
async function saveProfile() {setSaving(true);setMessage("");
...
if (loading) {return (<main style={styles.page}><section style={styles.card}><p style={styles.loading}>Loading doctor profile…</p></section></main>);}

return (<main style={styles.page}><section style={styles.card}><Link
       href="/dashboard"
       style={styles.back}
...
card: {maxWidth: 760,margin: "0 auto",background: "#ffffff",borderRadius: 28,padding: 28,boxShadow:"0 20px 60px rgba(15,23,42,.12)",},
...
subtitle: {fontSize: 20,color: "#526174",lineHeight: 1.45,},

heading: {fontSize: 28,fontWeight: 900,marginTop: 30,},

label: {display: "block",fontWeight: 800,marginTop: 16,marginBottom: 7,},

input: {width: "100%",boxSizing: "border-box",border: "2px solid #cbd5e1",borderRadius: 16,padding: 16,fontSize: 17,},

textarea: {width: "100%",boxSizing: "border-box",minHeight: 120,border: "2px solid #cbd5e1",borderRadius: 16,padding: 16,fontSize: 17,resize: "vertical",},

notice: {marginTop: 18,padding: 15,borderRadius: 14,background: "#fff7ed",color: "#9a3412",fontWeight: 700,lineHeight: 1.5,},

message: {marginTop: 20,padding: 15,borderRadius: 14,background: "#e0f2fe",color: "#075985",fontWeight: 800,whiteSpace: "pre-line",},

saveButton: {width: "100%",marginTop: 22,border: 0,borderRadius: 18,padding: 19,background: "#2563eb",color: "#ffffff",fontWeight: 900,fontSize: 20,cursor: "pointer",},

loading: {fontSize: 20,fontWeight: 800,},};
